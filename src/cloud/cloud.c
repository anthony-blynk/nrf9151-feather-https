...
void log_sockaddr_in(const struct sockaddr_in *addr)
{
    ...
    /* Port is network byte order → convert to host */
    uint16_t port = ntohs(addr->sin_port);

    LOG_INF("IPv4 %s:%u", ip, port);
}
static int socket_setup()
{
    ...
    log_sockaddr_in((struct sockaddr_in *)res->ai_addr);
